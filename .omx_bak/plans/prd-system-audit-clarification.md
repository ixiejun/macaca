...
- Architect pass tightened the plan by adding an explicit document-boundary contract, anti-drift rules, a single status decision for `ARCHITECTURE-v2.md`, and stronger current/intended/planned labeling requirements.
- Critic pass tightened the governance path by making this clarification lane explicitly outside OpenSpec, adding reproducible reader-answer and audit-traceability matrices, and adding the future pre-code gate: update OpenSpec artifacts, run strict validation, and confirm proposal approval before later architecture implementation.
