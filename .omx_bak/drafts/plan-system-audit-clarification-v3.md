...
- **Required future handoff condition before architecture implementation:** any later implementation run under `refactor-core-architecture` must update its proposal/tasks to cite the finalized docs and then run `openspec validate refactor-core-architecture --strict` before code changes start.
...
- **Risk:** future implementation starts without aligning the existing OpenSpec change to the clarified docs.
  - **Mitigation:** make `openspec validate refactor-core-architecture --strict` a pre-implementation gate for later architecture execution lanes, not for this clarification lane.
...
6. **Governance-path check**
   ...
   - Verify the PRD explicitly states that any later implementation under `refactor-core-architecture` must cite finalized docs and run `openspec validate refactor-core-architecture --strict` before code edits.
...
- Future refactor execution must explicitly absorb the clarified docs into the existing OpenSpec change before code edits begin.
