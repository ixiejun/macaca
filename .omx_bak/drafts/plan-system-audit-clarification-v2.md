...
- The active OpenSpec workstream `openspec/changes/refactor-core-architecture/` already depends on the audit’s architecture findings (`proposal.md:1-36`, `tasks.md:1-47`), but it does not yet carry documentation-baseline tasks.
...
6. The plan includes a concrete OpenSpec alignment decision: add documentation-baseline tasks to `openspec/changes/refactor-core-architecture/tasks.md` rather than opening a second overlapping architecture proposal.
7. A first-time reader can answer the user’s five required questions using the final docs alone:
...
**Approach:** Create a new canonical overview doc and narrow the audit, but leave README and OpenSpec alignment for later.
...
- attach documentation-baseline tasks to the existing `refactor-core-architecture` OpenSpec change.
**Pros:** Solves first-reader clarity, preserves the user’s requested two-layer docs, locks doc ownership boundaries, and aligns the docs baseline with the active refactor workstream.
...
- **Option B invalidated** because it leaves the stale README and OpenSpec alignment unresolved, so it fails the first-reader and execution-readiness goals together.
...
### Step 5 — Lock OpenSpec alignment
Update the active `openspec/changes/refactor-core-architecture/tasks.md` with documentation-baseline tasks rather than creating a second overlapping architecture proposal. The documentation clarification is a prerequisite planning layer for the existing change, not an independent architecture stream.

### Step 6 — Encode verification into the artifact set
Write:
...
Both artifacts must preserve the document-boundary contract, anti-drift rules, and OpenSpec alignment decision.
...
- **Risk:** OpenSpec and docs diverge.
  - **Mitigation:** treat doc-baseline work as part of `refactor-core-architecture` tasking, not a parallel proposal.
...
2. **Question-answerability check**
   - A reviewer with no prior context can answer the five required user questions using only README + overview + audit.
3. **Traceability check**
   - Every P0/P1 audit item in `SYSTEM_AUDIT.md` links to at least one overview principle/invariant anchor.
4. **Current/intended/planned labeling check**
   - Every target-state or planned statement in the overview is explicitly labeled, and no unlabeled speculative claims remain.
5. **Evidence check**
   - Key claims about current implementation still trace to source docs or code (`state.rs`, `routes.rs`, `audit.rs`) where applicable.
6. **OpenSpec alignment check**
   - `openspec/changes/refactor-core-architecture/tasks.md` contains doc-baseline tasks or an explicit recorded alignment note.
...
- The need to align docs with the already-active core-architecture refactor stream.

### Alternatives Considered
- Canonicalize a heavily rewritten `ARCHITECTURE-v2.md`.
- Split only overview/audit and defer README/OpenSpec work.

### Why Chosen
This option is the smallest change set that resolves reader clarity, doc ownership boundaries, and execution traceability at the same time.

### Consequences
- A new canonical doc is introduced.
- `ARCHITECTURE-v2.md` must carry explicit non-canonical status.
- Existing refactor planning gains a clearer narrative baseline.

### Follow-ups
- Add doc-baseline tasks to `openspec/changes/refactor-core-architecture/tasks.md`.
...
- `verifier` (high): run newcomer-question, traceability, and anti-drift checks.
...
- Lane 4 — `verifier` (high): verify sections, labels, links, and OpenSpec alignment.
- Team verification path: team proves file contents and link integrity; verifier/Ralph proves the five reader questions and audit-to-principle traceability.
