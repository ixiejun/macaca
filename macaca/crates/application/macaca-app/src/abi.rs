...
use macaca_proto::{
    ...
    validate_developer_repository_permission_declarations,
    ...
    validate_location_place_search_permission_declarations,
    validate_location_timezone_permission_declarations, validate_secret_reference_declarations,
...
impl ApplicationAbiAdapter for YamlApplicationAbiAdapter {
    fn load(&self) -> Result<ApplicationAbiLoadResult, ApplicationAbiError> {
        ...
        if let Some(service_contract) = self.manifest.service_contract.as_ref() {
            ...
            validate_secret_reference_declarations(service_contract).map_err(|reason| {
                warn!(application_id = %application_id, reason, "application ABI secret reference admission rejected");
                ApplicationAbiError::InvalidDeclaration(reason.into())
            })?;
            ...
            validate_location_timezone_permission_declarations(service_contract).map_err(|reason| {
                warn!(application_id = %application_id, reason, "location timezone permission admission rejected");
                ApplicationAbiError::InvalidDeclaration(reason.into())
            })?;
            ...
            validate_commerce_payment_intent_permission_declarations(service_contract).map_err(|reason| {
...
#[cfg(test)]
#[path = "abi_foundation_secrets_reference_tests.rs"]
