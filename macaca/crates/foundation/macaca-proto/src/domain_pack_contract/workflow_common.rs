...
/// Declarative provider-class row used by Workflow child-pack descriptors.
///
/// Provider classes describe replaceable workflow capability shapes such as
/// durable engine, remote workflow service, plugin, mock, and unavailable. They
/// are descriptor metadata only and must never become branches on concrete
/// provider names, application names, task queues, worker names, or business
/// workflows inside proto, SDK, kernel, shell, or generic application code.
#[derive(Debug, Clone, Copy, Serialize)]
pub(crate) struct WorkflowProviderClass<'a> {
    pub(crate) provider_class: &'a str,
    pub(crate) availability: DomainPackProviderCapabilityState,
    pub(crate) metadata: &'a [(&'a str, &'a str)],
}

/// Descriptor input for one Workflow sub-pack.
///
/// This Bridge-pattern helper keeps descriptor assembly data-driven while child
/// modules own their Command-pattern DTOs. New workflow packs should extend
/// declarative metadata and tests, not add alternate execution paths or embed
/// application-specific orchestration semantics.
pub(crate) struct WorkflowPackDescriptor<'a> {
    ...
    pub(crate) provider_classes: &'a [WorkflowProviderClass<'a>],
...
/// Build a descriptor-only Workflow pack definition.
///
/// Workflow packs coordinate long-running autonomous work. Descriptors remain
/// preview-unavailable until approved service providers register through the
/// canonical service runtime with policy, entitlement, resource, approval,
/// trace, audit, redaction, health, snapshot, and unavailable-state decorators.
pub(crate) fn workflow_pack_definition(spec: WorkflowPackDescriptor<'_>) -> DomainPackDefinition {
    ...
    DomainPackDefinition::with_metadata(
        spec.pack_id,
        DomainPackMetadata {
            family_id: "workflow".into(),
            parent_pack_id: Some("pack.workflow.v1".into()),
            version: "v1".into(),
            ...
            sdk: DomainPackSdkMetadata {
                client_namespace: format!("sdk.packs.workflow.{}", spec.sdk_slug),
                docs_url: format!("docs://macaca/developer-packs/workflow/{}", spec.docs_slug),
                examples: spec
                    .examples
                    .iter()
...
fn provider_descriptor(
    spec: &WorkflowPackDescriptor<'_>,
    provider: &WorkflowProviderClass<'_>,
    ...
    let capability = GenericWorkflowProviderCapability {
    ...
    DomainPackProviderDescriptor {
        ...
        availability: provider.availability,
        capability_hash: workflow_stable_hash(&capability),
        compatibility_hash: format!("workflow-{}-provider-v1", spec.docs_slug),
...
#[derive(Serialize)]
struct GenericWorkflowProviderCapability<'a> {
    pack_id: &'a str,
...
/// Compute deterministic, trace-safe compatibility evidence for Workflow DTOs.
///
/// This hash is audit identity, not cryptography. Inputs must stay bounded and
/// reference-only: raw prompts, raw task payloads, raw artifacts, package bytes,
/// credentials, provider payloads, worker logs, replay exports, and unbounded
/// histories must never enter hash evidence.
pub(crate) fn workflow_stable_hash<T: Serialize + ?Sized>(value: &T) -> String {
...
/// Opaque Workflow command envelope shared by typed command wrappers.
...
/// Bounded page shared by Workflow result DTOs.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// Sanitized Workflow error payload for trace-safe diagnostics.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
...
macro_rules! define_workflow_command_wrappers {
    ($($name:ident),+ $(,)?) => {
        $(
            #[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
            pub struct $name {
                pub request: WorkflowCommandEnvelope,
            }
        )+
    };
}

pub(crate) use define_workflow_command_wrappers;
