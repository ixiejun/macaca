...
/// Declarative provider-class row used by Device child-pack descriptors.
///
/// Provider classes identify replaceable host capability shapes such as
/// host-native, browser, remote-host, plugin, mock, and unavailable. They are
/// descriptor metadata, not platform routing logic. Runtime-host and plugin
/// composition roots own concrete host API bindings after policy, resource,
/// entitlement, trace, audit, foreground/background, and redaction decorators
/// have been installed.
#[derive(Debug, Clone, Copy)]
pub(crate) struct DeviceProviderClass<'a> {
    pub(crate) provider_class: &'a str,
    pub(crate) availability: DomainPackProviderCapabilityState,
    pub(crate) metadata: &'a [(&'a str, &'a str)],
}

/// Descriptor input for one Device sub-pack.
///
/// This Bridge-pattern helper keeps pack construction data-driven while child
/// modules own their Command-pattern DTOs. Adding a new Device pack should add
/// declarative data and tests, not branch on host platform, model, application,
/// workflow, provider, service type, or permission prompt names.
pub(crate) struct DevicePackDescriptor<'a> {
    ...
    pub(crate) provider_classes: &'a [DeviceProviderClass<'a>],
...
/// Build a descriptor-only Device pack definition.
///
/// Device capabilities can expose host hardware, host files, user-visible
/// notifications, foreground/background lifecycle, and private local context.
/// Descriptors therefore remain preview-unavailable until an approved service
/// provider registers matching schemas through the canonical service runtime.
pub(crate) fn device_pack_definition(spec: DevicePackDescriptor<'_>) -> DomainPackDefinition {
    ...
    DomainPackDefinition::with_metadata(
        ...
        DomainPackMetadata {
            ...
            sdk: DomainPackSdkMetadata {
                client_namespace: format!("sdk.packs.device.{}", spec.sdk_slug),
                docs_url: format!("docs://macaca/developer-packs/device/{}", spec.docs_slug),
                examples: spec
                    .examples
                    .iter()
...
fn provider_descriptor(
    spec: &DevicePackDescriptor<'_>,
    provider: &DeviceProviderClass<'_>,
    ...
    let capability = GenericDeviceProviderCapability {
    ...
    DomainPackProviderDescriptor {
        ...
        availability: provider.availability,
        capability_hash: device_stable_hash(&capability),
        compatibility_hash: format!("device-{}-provider-v1", spec.docs_slug),
...
#[derive(Serialize)]
struct GenericDeviceProviderCapability<'a> {
    pack_id: &'a str,
...
/// Compute deterministic, trace-safe compatibility evidence for Device DTOs.
///
/// This hash is audit identity, not cryptography. Inputs must stay bounded and
/// reference-only: raw sensor samples, raw camera frames, raw media bytes, raw
/// local file paths, raw file contents, notification bodies, push tokens, host
/// identifiers, provider payloads, credentials, prompts, and unbounded
/// lifecycle logs must never enter hash evidence.
pub(crate) fn device_stable_hash<T: Serialize + ?Sized>(value: &T) -> String {
...
/// Validate bounded device references before they enter trace or SDK diagnostics.
pub(crate) fn bounded_device_token(value: &str, max_len: usize) -> bool {
    !value.is_empty()
        && value.len() <= max_len
...
/// Opaque Device command envelope shared by typed command wrappers.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct DevicePackCommandEnvelope {
...
/// Bounded page shared by Device result DTOs.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct DevicePackPage<T> {
    pub items: Vec<T>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Sanitized Device error payload for trace-safe diagnostics.
#[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
pub struct DevicePackError {
    pub code: String,
    pub message: String,
    pub retryable: bool,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
macro_rules! define_device_command_wrappers {
    ($($name:ident),+ $(,)?) => {
        $(
            #[derive(Debug, Clone, Default, PartialEq, Eq, Serialize, Deserialize)]
            pub struct $name {
                pub request: DevicePackCommandEnvelope,
            }
        )+
    };
}

pub(crate) use define_device_command_wrappers;
