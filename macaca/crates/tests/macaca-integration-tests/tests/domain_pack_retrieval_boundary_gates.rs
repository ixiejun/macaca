//! Vector stores and rerank engines must remain runtime-host provider details.
...
const FORBIDDEN: &[&str] = &[
    "RetrievalSystemServiceProvider",
    "retrieval_service_provider",
    "PineconeRetrievalProvider",
    "WeaviateRetrievalProvider",
...
#[test]
fn retrieval_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "retrieval provider boundary violations:\n{}",
        violations.join("\n")
    );
}
