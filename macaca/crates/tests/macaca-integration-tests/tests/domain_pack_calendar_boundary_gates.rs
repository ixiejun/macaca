//! Calendar connector implementations must remain runtime-host details.
...
#[test]
fn calendar_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "calendar provider boundary violations:\n{}",
        violations.join("\n")
    );
}
