//! Host-lifecycle provider ownership and no-direct-call gates.
...
const PROVIDER_TOKENS: &[&str] = &[
    "DeviceHostLifecycleSystemServiceProvider",
    "device_host_lifecycle_service_provider",
    "host_lifecycle_adapter",
];
const DIRECT_SURFACES: &[&str] = &[
    "crates/facade/macaca-sdk/src",
...
#[test]
fn host_lifecycle_providers_stay_outside_provider_neutral_layers() {
    assert_no_tokens(
        PROVIDER_NEUTRAL,
        PROVIDER_TOKENS,
        "host lifecycle provider boundary violations",
    );
}
#[test]
fn host_lifecycle_sdk_and_wasm_imports_cannot_bypass_service_runtime() {
    assert_no_tokens(
        DIRECT_SURFACES,
        DIRECT_TOKENS,
        "host lifecycle direct-provider bypasses",
    );
}
