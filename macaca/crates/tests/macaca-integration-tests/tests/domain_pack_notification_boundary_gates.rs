//! Notification provider boundary gate.
//!
//! Provider-neutral notification DTOs may cross OS layers, while concrete
//! runtime-host notification providers must remain behind composition roots.
...
#[test]
fn notification_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "notification provider boundary violations:\n{}",
        violations.join("\n")
    );
}
