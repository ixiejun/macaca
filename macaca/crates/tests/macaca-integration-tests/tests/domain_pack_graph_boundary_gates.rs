//! Graph engines and provider-specific adapters must remain runtime-host details.
...
const FORBIDDEN: &[&str] = &[
    "GraphSystemServiceProvider",
    "graph_service_provider",
    "Neo4jGraphProvider",
    "NeptuneGraphProvider",
...
#[test]
fn graph_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "graph provider boundary violations:\n{}",
        violations.join("\n")
    );
}
