//! Summarization engines and provider adapters must remain runtime-host details.
...
const FORBIDDEN: &[&str] = &[
    "SummarizationSystemServiceProvider",
    "summarization_service_provider",
    "OpenAISummarizationProvider",
    "AnthropicSummarizationProvider",
    "AzureSummarizationProvider",
    "SummaryModelAdapter",
];
...
#[test]
fn summarization_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "summarization provider boundary violations:\n{}",
        violations.join("\n")
    );
}
