...
/// A stable protocol microkernel architecture layer.
///
...
/// protocol microkernel treats them as replaceable service providers because their concrete
...
///
/// The protocol microkernel boundary gate audits production dependency ownership. Dev-only
...
/// giant nested conditional block. Future protocol microkernel phases can add new
...
/// A temporary migration exception for known protocol microkernel debt.
///
...
impl AllowlistEntry {
    /// Builds one dependency-debt entry with all fields needed for deterministic
...
fn boundary_rules() -> Vec<BoundaryRule> {
    vec![
        ...
        BoundaryRule {
            ...
            rationale: "The SDK/SystemFacade may expose typed clients and bounded composition DTOs, but it must not depend on presentation shells, persistence construction, or other provider-factory hubs. Approved §9.4 edges: macaca-sdk → macaca-app and macaca-sdk → macaca-runtime-host exist only while focused clients absorb the remaining composition surface.",
            replacement: "return focused service clients or Null Object unavailable clients; keep provider factories in approved runtime-host composition roots; move shell type imports to stable SDK client or protocol DTO modules",
            matches: |edge| {
...
fn run_cargo_metadata() -> Value {
    eprintln!(
        "protocol_service_dependency_gate event=metadata_start workspace={}",
    ...
    eprintln!(
        "protocol_service_dependency_gate event=metadata_complete bytes={}",
        output.stdout.len()
    );
    ...
    for package in packages {
            ...
            .expect("dependencies should be an array")
        {
            ...
            if !kind.is_production() {
                eprintln!(
                    "protocol_service_dependency_gate event=skip_dev_edge from={} to={}",
                    from, to
                );
                continue;
            }
            ...
            if dependency["optional"].as_bool() == Some(true) {
                eprintln!(
                    "protocol_service_dependency_gate event=skip_optional_edge from={} to={}",
                    from, to
                );
                continue;
            }
...
fn assert_every_workspace_crate_is_classified(metadata: &Value) {
    ...
    assert!(
        unknown.is_empty(),
        "protocol microkernel dependency gate found unclassified workspace crates: {unknown:?}. \
    ...
    for edge in edges {
        ...
        for rule in &rules {
            ...
            if let Some(entry) = allowed.get(&(rule.id, classified.from, classified.to)) {
                eprintln!(
                    "protocol_service_dependency_gate event=allowlisted rule={} from={} to={} owner_track={} current_caller={} phase={} replacement={} validation_command={}",
                    entry.rule_id,
                    entry.from,
                    entry.to,
                    entry.owner_track,
...
/// Terminal-state invariant for protocol microkernel dependency debt (P5 §6.1.1).
///
...
pub fn assert_protocol_service_allowlist_terminal_state() {
    let entries = allowlist::entries();
    let row_count = entries.len();
    eprintln!("protocol_service_dependency_gate event=terminal_allowlist_check rows={row_count}");
    if entries.is_empty() {
        eprintln!(
            "protocol_service_dependency_gate event=terminal_allowlist_pass reason=zero_rows"
        );
        return;
    }

    let mut diagnostics = String::from(
        "protocol microkernel terminal gate failed: dependency allowlist must be empty at terminal state.\n\
...
pub fn assert_protocol_service_dependency_boundaries() {
    ...
    assert_protocol_service_allowlist_terminal_state();
    ...
    eprintln!(
        "protocol_service_dependency_gate event=edge_visit_complete edges={}",
        edges.len()
    );
    let violations = evaluate_edges(&edges);
    assert!(
        violations.is_empty(),
        "protocol microkernel dependency boundary violations were found:{}",
