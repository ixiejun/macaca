//! Citation implementation engines must remain runtime-host details.
...
const FORBIDDEN: &[&str] = &[
    "CitationSystemServiceProvider",
    "citation_service_provider",
    "CrossrefCitationProvider",
    "DataCiteCitationProvider",
    "CslStyleRenderer",
    "IdentifierResolverAdapter",
];
fn root() -> PathBuf {
...
#[test]
fn citation_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "citation provider boundary violations:\n{}",
        violations.join("\n")
    );
}
