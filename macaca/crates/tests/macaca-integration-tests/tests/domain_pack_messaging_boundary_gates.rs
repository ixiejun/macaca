//! Messaging connector implementations must remain runtime-host details.
...
const FORBIDDEN: &[&str] = &[
    "MessagingSystemServiceProvider",
...
#[test]
fn messaging_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "messaging provider boundary violations:\n{}",
        violations.join("\n")
    );
}
