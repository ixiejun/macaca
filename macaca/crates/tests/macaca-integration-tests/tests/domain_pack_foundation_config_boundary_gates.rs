//! Foundation-config provider boundary gates.

use std::fs;
use std::path::{Path, PathBuf};

const PROVIDERS: &[&str] = &["MockConfigProvider", "UnavailableConfigProvider"];

#[test]
fn provider_neutral_layers_do_not_construct_foundation_config_providers() {
    let root = root();
    let surfaces = [
        "crates/kernel",
...
#[test]
fn config_sdk_and_wasm_imports_cannot_bypass_service_runtime() {
    let root = root();
    assert_no_tokens(
        &root,
        [
            root.join("crates/facade/macaca-sdk/src/foundation_config_client.rs"),
            root.join(
    ...
    I: IntoIterator<Item = PathBuf>,
{
    ...
    assert!(
        violations.is_empty(),
        "foundation config boundary violations:\n{}",
        violations.join("\n")
    );
}
