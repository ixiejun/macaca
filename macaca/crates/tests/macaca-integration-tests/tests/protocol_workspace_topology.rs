//! protocol microkernel workspace topology guard.
//!
//! This test is executable architecture documentation. protocol microkernel no longer treats
...
//! live in the expected protocol microkernel layer.
...
/// protocol microkernel `crates/tests/` layer.
...
/// new crates cannot enter the repository without declaring their protocol microkernel layer
...
fn expected_manifest_suffixes() -> BTreeMap<&'static str, &'static str> {
    BTreeMap::from([
        ...
        (
            ...
            "crates/packages/macaca-domain-pack-finance/Cargo.toml",
        ),
    ])
}
...
#[test]
fn protocol_service_workspace_topology_matches_expected_layers() {
    ...
    assert!(
        unknown.is_empty(),
        "unknown workspace package(s) missing protocol microkernel topology classification: {unknown:?}. \
Update OpenSpec, macaca/crates/README.md, and protocol_service_workspace_topology.rs before relying on new crates."
    );
    ...
    assert!(
        missing.is_empty(),
        "expected protocol microkernel workspace package(s) missing from cargo metadata: {missing:?}"
    );
    ...
    assert!(
        mismatches.is_empty(),
        "protocol microkernel workspace topology mismatch:\n{}",
        mismatches.join("\n")
    );
}
