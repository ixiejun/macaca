...
//! backends (retired authority markers and lifecycle suppression patches).
//!
...
pub fn assert_coordination_patch_retirement_gate() {
    eprintln!("p5_terminal_audit_gate event=coordination_patch_retirement_start gate={GATE_ID}");
    assert_token_family_gate(GATE_ID, FAMILIES, None);
    eprintln!("p5_terminal_audit_gate event=coordination_patch_retirement_pass gate={GATE_ID}");
}
