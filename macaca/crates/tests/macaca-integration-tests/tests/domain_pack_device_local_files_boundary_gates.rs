//! Local-files provider ownership and direct-call boundary gates.
...
const PROVIDER_TOKENS: &[&str] = &[
    "DeviceLocalFilesSystemServiceProvider",
    "device_local_files_service_provider",
    "host_file_adapter",
    "FileSystemAccessHandle",
    "SecurityScopedFileHandle",
];
...
#[test]
fn local_file_providers_stay_outside_provider_neutral_layers() {
    assert_no_tokens(PROVIDER_NEUTRAL, PROVIDER_TOKENS);
}

#[test]
fn local_file_commands_cannot_bypass_typed_service_runtime() {
    assert_no_tokens(DIRECT_CALL_SURFACES, DIRECT_CALL_TOKENS);
}

fn assert_no_tokens(surfaces: &[&str], forbidden: &[&str]) {
