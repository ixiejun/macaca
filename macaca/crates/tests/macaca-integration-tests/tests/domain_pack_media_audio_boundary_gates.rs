//! Media-audio provider ownership and direct-call boundary gates.
...
const FORBIDDEN_PROVIDER_TOKENS: &[&str] = &[
    "MediaAudioSystemServiceProvider",
    "media_audio_service_provider",
...
#[test]
fn audio_boundaries_do_not_import_or_construct_concrete_providers() {
    assert_no_tokens(
        PROVIDER_NEUTRAL_SURFACES,
        FORBIDDEN_PROVIDER_TOKENS,
        "audio provider boundary violations",
    );
}

#[test]
fn audio_sdk_and_wasm_imports_cannot_bypass_service_runtime() {
    assert_no_tokens(
        DIRECT_CALL_SURFACES,
        FORBIDDEN_DIRECT_CALL_TOKENS,
        "audio direct-provider bypasses",
    );
}
