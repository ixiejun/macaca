//! Email connector implementations must remain runtime-host details.
...
#[test]
fn email_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "email provider boundary violations:\n{}",
        violations.join("\n")
    );
}
