//! Search engines and index adapters must remain runtime-host details.
...
#[test]
fn search_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "search provider boundary violations:\n{}",
        violations.join("\n")
    );
}
