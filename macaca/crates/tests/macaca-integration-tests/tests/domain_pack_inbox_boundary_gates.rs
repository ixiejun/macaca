//! Inbox connector implementations must remain runtime-host details.
...
const FORBIDDEN: &[&str] = &[
    "InboxSystemServiceProvider",
    "inbox_service_provider",
    "Gmail",
    "Microsoft Graph",
    "IMAP",
    "Slack",
...
#[test]
fn inbox_boundaries_do_not_import_or_construct_concrete_providers() {
    let root = root();
    ...
    assert!(
        violations.is_empty(),
        "inbox provider boundary violations:\n{}",
        violations.join("\n")
    );
}
