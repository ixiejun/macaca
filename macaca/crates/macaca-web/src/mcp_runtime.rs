...
//!
//! This module keeps MCP server definitions and runtime probing outside any
//! single application or skill. Framework protocol handling stays in
//! `macaca-framework::mcp`; this module owns Agent OS policy, status and
...
/// Resolve MCP definitions declared by a visible skill snapshot.
pub fn definitions_from_skill_snapshot(snapshot: &SkillSnapshot) -> Vec<McpServerDefinition> {
    ...
    for skill in &snapshot.skills {
        for server in &skill.mcp_servers {
            if let Some(definition) = definition_from_skill_server(skill, server) {
                if seen.insert(definition.id.clone()) {
                    definitions.push(definition);
                }
            }
        }
        if let Some(definition) = compatibility_definition_from_skill(skill) {
            if seen.insert(definition.id.clone()) {
                definitions.push(definition);
            }
        }
...
fn definition_from_skill_server(
    ...
    Some(McpServerDefinition {
        ...
        transport: McpTransportConfig::Stdio {
            ...
            args: ensure_playwright_isolated_args(&server.command, server.args.clone()),
            env: BTreeMap::new(),
...
fn compatibility_definition_from_skill(skill: &SkillSnapshotEntry) -> Option<McpServerDefinition> {
    ...
    let mut definition = playwright_compat_definition();
    definition.id = format!("skill:{}:playwright", skill.name);
    definition.source = McpDefinitionSource::Compatibility;
    Some(definition)
}

fn ensure_playwright_isolated_args(command: &str, mut args: Vec<String>) -> Vec<String> {
    if command.contains("playwright")
        && !args.iter().any(|arg| arg == "--isolated")
...
/// Build the built-in compatibility definition for Playwright MCP.
pub fn playwright_compat_definition() -> McpServerDefinition {
    McpServerDefinition {
...
mod tests {
    use super::*;
    use macaca_skill::{SkillInstallSpec, SkillSnapshot};
...
"#,
    ...
    #[test]
    fn playwright_compat_uses_isolation() {
        let definition = playwright_compat_definition();
