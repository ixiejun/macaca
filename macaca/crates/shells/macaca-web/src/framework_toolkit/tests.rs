...
use macaca_host_composition::mcp_runtime::{
    McpDefinitionSource, McpLifecycleScope, McpRuntimeStatus, McpRuntimeStatusState,
    McpServerDefinition,
};
...
fn resolve_workspace_path_joins_relative_path_to_workspace_root() {
    ...
    let resolved = macaca_host_composition::tool_bootstrap::resolve_workspace_tool_path(
        workspace_root,
        "shared/service/main.go",
    );
...
fn normalize_tool_input_parses_stringified_json_object() {
    ...
    let normalized =
        macaca_host_composition::tool_bootstrap::normalize_workspace_tool_input(&input);
...
#[test]
fn mcp_runtime_event_plan_preserves_lifecycle_events() {
    let statuses = vec![
        mcp_status(
            "server-ready",
...
fn mcp_definition(id: &str, enabled: bool) -> McpServerDefinition {
    McpServerDefinition {
        id: id.into(),
