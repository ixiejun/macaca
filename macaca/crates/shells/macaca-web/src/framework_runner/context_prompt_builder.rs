...
use macaca_host_composition::context::{
    ContextSourceKind, PromptComposer, PromptStability, TrustLevel,
};
...
pub(crate) async fn build_context_system_prompt(
    ...
    // Inject capabilities from the framework construction capability contract.
    let flattened = capabilities.flatten();
    ...
    if let Some(ref dir) = app_dir {
        for section in load_workspace_guide_sections(dir, &merged_context.workspace_guides).await {
