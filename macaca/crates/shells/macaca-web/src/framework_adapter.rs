...
use macaca_sdk::framework::message::{ContentBlock, TextBlock, ThinkingBlock, ToolUseBlock};
use macaca_sdk::framework::model::{ChatModel, ChatOptions, ChatResponse, ChatUsage, ModelError};
use macaca_sdk::framework::tool::{ToolError, ToolHandler, ToolResponse, ToolTraceEvent, Toolkit};
...
pub(crate) struct ServiceChatModelAdapter {
    ...
    scope: macaca_sdk::llm::LlmServiceScope,
}
...
impl ChatModel for ServiceChatModelAdapter {
    async fn chat(
        ...
        let llm_messages = macaca_sdk::framework::llm_wire::messages_from_json_values(&messages);
        ...
        let command = macaca_sdk::llm::LlmChatCommand::new(
            self.scope.clone(),
            trace,
            llm_messages,
            llm_options,
...
pub(crate) struct SingleToolAdapter {
    tool: Box<dyn macaca_sdk::tools::Tool>,
}

impl SingleToolAdapter {
    pub(crate) fn new(tool: Box<dyn macaca_sdk::tools::Tool>) -> Self {
        Self { tool }
    }
}
...
impl ToolHandler for SingleToolAdapter {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResponse, ToolError> {
        let result = macaca_sdk::tools::ToolCommandExecutor::execute_command(
            self.tool.as_ref(),
            macaca_sdk::tools::ToolCommand::new(args),
        )
        .await
    ...
    async fn execute_streaming(
        ...
        let command = if let Some(event_tx) = event_tx {
            ...
            macaca_sdk::tools::ToolCommand::with_context(
                args,
                macaca_sdk::tools::ToolCommandContext {
        ...
        let result =
            macaca_sdk::tools::ToolCommandExecutor::execute_command(self.tool.as_ref(), command)
                .await
                .map_err(|error| ToolError::ExecutionFailed(error.to_string()))?;
        Ok(ToolResponse::text(
            serde_json::to_string(&result).unwrap_or_else(|_| result.to_string()),
    ...
    fn schema(&self) -> serde_json::Value {
        macaca_sdk::tools::ToolSchemaProvider::tool_schema(self.tool.as_ref())
    }
}
...
impl ToolSetBridge {
    pub(crate) fn from_tool_set(tool_set: Arc<dyn macaca_sdk::tools::ToolCatalog>) -> Toolkit {
        let mut toolkit = Toolkit::new();
        for definition in macaca_sdk::tools::ToolCatalog::definitions(tool_set.as_ref()) {
            toolkit.register(
...
struct CatalogToolAdapter {
    ...
    tool_set: Arc<dyn macaca_sdk::tools::ToolCatalog>,
}
...
impl ToolHandler for CatalogToolAdapter {
    async fn execute(&self, args: serde_json::Value) -> Result<ToolResponse, ToolError> {
        let tool =
            macaca_sdk::tools::ToolCatalog::find_tool(self.tool_set.as_ref(), &self.tool_name)
...
fn tool_trace_from_sdk(event: macaca_sdk::tools::TraceEvent) -> ToolTraceEvent {
    ToolTraceEvent {
