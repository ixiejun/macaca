//! Anthropic Messages API formatter (**Strategy** / Adapter).
//!
//! Handles system-prompt extraction, typed content arrays, and tool_use /
//! tool_result blocks in the Messages API wire shape.

use serde_json::{json, Value};
use tracing::{debug, warn};

use crate::message::{
    ContentBlock, Msg, MsgContent, Role, TextBlock, ThinkingBlock, ToolResultBlock,
    ToolUseBlock,
};
...
impl Formatter for AnthropicFormatter {
    fn format(&self, msgs: &[Msg]) -> Vec<Value> {
        debug!(
            target: "macaca_framework::formatter::anthropic",
            message_count = msgs.len(),
            "AnthropicFormatter::format entry — encoding non-system messages for Messages API"
        );
        msgs.iter()
    ...
    fn parse_response(&self, raw: Value) -> Result<ChatResponse, FormatterError> {
        debug!(
            target: "macaca_framework::formatter::anthropic",
            "AnthropicFormatter::parse_response entry — decoding Messages API JSON"
        );
        let id = raw
            .get("id")
            .and_then(|v| v.as_str())
        ...
        let content_arr = raw
            ...
            .ok_or_else(|| {
                ...
                FormatterError::Parse("missing content array".into())
            })?;
        ...
        debug!(
            ...
            "AnthropicFormatter::parse_response succeeded"
        );
        Ok(response)
    }
}
