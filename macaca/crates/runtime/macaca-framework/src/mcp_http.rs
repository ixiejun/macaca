...
//! HTTP transport adapter for framework-level MCP clients.
//!
//! The adapter implements the framework MCP client trait without becoming an OS
//! service provider. Runtime-host composition can still replace this transport
//! with plugin, remote, mock, or unavailable implementations.
...
mod enabled {
    ...
    /// variant is represented in the same client abstraction so Agent OS config
    /// can resolve it uniformly; full bidirectional SSE session handling can be
    ...
    impl McpClient for HttpMcpClient {
        async fn connect(&mut self) -> Result<(), McpError> {
            ...
            if self.transport == HttpMcpTransport::Sse {
                debug!(url = %self.url, "initializing MCP SSE client via HTTP request path");
            } else {
                debug!(url = %self.url, "initializing MCP streamable HTTP client");
            }
            ...
            self.send_notification("notifications/initialized", None)
                .await?;
            self.connected = true;
            info!("MCP HTTP client initialized");
            Ok(())
        }
        ...
        async fn list_tools(&mut self) -> Result<Vec<McpToolDef>, McpError> {
            ...
            let tools: Vec<McpToolDef> = serde_json::from_value(tools_value)
                .map_err(|e| McpError::Protocol(e.to_string()))?;
            debug!(tool_count = tools.len(), "MCP HTTP client listed tools");
            Ok(tools)
        }
    ...
    pub(crate) fn parse_http_mcp_response(
        content_type: &str,
        body: &str,
    ) -> Result<Value, McpError> {
...
#[cfg(feature = "mcp-http")]
pub use enabled::{HttpMcpClient, HttpMcpTransport};

#[cfg(all(test, feature = "mcp-http"))]
pub(crate) use enabled::parse_http_mcp_response;
