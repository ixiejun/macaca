//! Stdio MCP client — JSON-RPC 2.0 over child-process stdin/stdout (**Adapter**).
//!
//! Spawns a configured command, performs MCP initialize handshake, and multiplexes
//! request/response lines on the process pipes with timeout guards.
...
impl StdioMcpClient {
    ...
    async fn send_request(
        ...
        tracing::debug!(
            target = "macaca_framework::mcp::stdio",
            method = method,
            request_id = id,
            "sending stdio MCP JSON-RPC request"
        );
            ...
            if parsed.get("id").is_some() {
                if let Some(err) = parsed.get("error") {
                    ...
                    tracing::warn!(
                        target = "macaca_framework::mcp::stdio",
                        error = msg,
                        "stdio MCP JSON-RPC error response"
                    );
    ...
    pub async fn call_tool_mut(
        ...
        tracing::debug!(
            target = "macaca_framework::mcp::stdio",
            tool = name,
            "calling MCP tool via stdio transport"
        );
...
impl McpClient for StdioMcpClient {
    async fn connect(&mut self) -> Result<(), McpError> {
        ...
        tracing::debug!(
            ...
            "spawning stdio MCP child process"
        );
