...
//!
//! WASM host imports and YAML workflow steps enter through the Application
...
//! runtime package kind. The Bridge pattern keeps shell adapters small while
//! preserving provider-neutral intent selection via metadata.
...
pub const DEFAULT_AGENT_DELEGATE_WAIT_MS: u64 = 30_000;

/// Service-bus source recorded when the orchestration adapter forwards work.
pub const ORCHESTRATION_AGENT_EXECUTION_SOURCE: &str =
    "macaca.host_composition.application_agent_delegate_bridge";
...
pub fn build_execution_command_from_delegate(
...
pub async fn dispatch_agent_execution_via_service(
...
pub fn delegate_result_from_execution_reply(
    command: &ApplicationAgentDelegateCommand,
    app_id: ApplicationId,
    session_id: String,
    task_id: TaskId,
    ...
    Ok(ApplicationAgentDelegateResult {
        ...
        task_id: result
            ...
            .or_else(|| Some(task_id.0.to_string())),
