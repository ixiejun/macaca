...
fn local_governance_store_logs_key_audit_nodes_with_structured_fields() {
    ...
    // the service split; snapshot logs remain on the provider state module.
    ...
    assert!(governance_store_source
        .contains("skill governance event appended through local governance store adapter"));
    ...
    assert!(governance_store_source
        .contains("skill governance read model replayed through local governance store adapter"));
    ...
    assert!(state_source
        .contains("skill governance snapshot built through local governance store adapter"));
