//! Optional service host bootstrap helpers for presentation-shell thinness.
//!
...
use macaca_persist::{EntitlementStore, EventLog, PaymentStore};
use macaca_proto::{KernelServiceId, MacacaError, MacacaResult, PaymentTerms, TraceContext};
use tracing::info;

use crate::{
    entitlement::RuntimeEntitlementGuard, entitlement_service_descriptor, evm_service_descriptor,
    payment_service_descriptor, store_service_descriptor, web3_service_descriptor,
...
#[derive(Clone)]
pub struct OptionalServiceBootstrapInputs {
    pub entitlement_store: Arc<dyn EntitlementStore>,
    pub entitlement_event_log: Option<Arc<EventLog>>,
    pub payment_store: Arc<dyn PaymentStore>,
    pub payment_terms: PaymentTerms,
}

impl OptionalServiceBootstrapInputs {
    /// Build optional service bootstrap inputs from explicit repositories.
    ///
...
pub struct OptionalServiceBootstrapDiagnostic {
...
impl OptionalServiceBootstrapDiagnostic {
    fn completed(
...
pub struct OptionalServiceRuntimeBundle {
    pub started_services: Vec<KernelServiceId>,
    pub diagnostics: Vec<OptionalServiceBootstrapDiagnostic>,
}

/// Builder that registers and starts optional service families.
pub struct OptionalServiceBootstrap {
    runtime: Arc<ServiceRuntime>,
    inputs: OptionalServiceBootstrapInputs,
    trace_prefix: String,
}

impl OptionalServiceBootstrap {
    ...
    pub async fn bootstrap(self) -> MacacaResult<OptionalServiceRuntimeBundle> {
        info!(
            trace_prefix = %self.trace_prefix,
            "optional services bootstrap started"
        );
        let mut bundle = OptionalServiceRuntimeBundle::default();
        let entitlement_guard = match &self.inputs.entitlement_event_log {
            Some(event_log) => Arc::new(RuntimeEntitlementGuard::with_event_log(
                Arc::clone(&self.inputs.entitlement_store),
                Arc::clone(event_log),
            )),
            None => Arc::new(RuntimeEntitlementGuard::new(Arc::clone(
                &self.inputs.entitlement_store,
            ))),
        };

        self.register_and_start(
            entitlement_service_descriptor(),
            Arc::new(EntitlementSystemServiceProvider::with_guard(
                Arc::clone(&self.inputs.entitlement_store),
                Arc::clone(&entitlement_guard),
            )),
            "entitlement-service",
            &mut bundle,
        )
        ...
        self.register_and_start(
            ...
            Arc::new(StoreSystemServiceProvider::with_entitlement_guard(
                Arc::clone(&entitlement_guard),
            )),
            "store-service",
            &mut bundle,
        ...
        info!(
            ...
            "optional services bootstrap completed"
        );
        Ok(bundle)
    }
    ...
    async fn register_and_start(
        ...
        bundle: &mut OptionalServiceRuntimeBundle,
    ) -> MacacaResult<()> {
        ...
        info!(
            ...
            "optional services registering provider"
        );
        self.runtime
            .register_provider(
        ...
        info!(
            service_id = %service_id,
            trace_id = %trace.trace_id,
            "optional services starting provider"
        );
        self.runtime
        ...
        bundle
            .diagnostics
            .push(OptionalServiceBootstrapDiagnostic::completed(
                service_id,
                "register_start",
                &trace,
            ));
...
pub async fn bootstrap_optional_services(
    runtime: Arc<ServiceRuntime>,
    inputs: OptionalServiceBootstrapInputs,
    trace_prefix: impl Into<String>,
) -> MacacaResult<OptionalServiceRuntimeBundle> {
    OptionalServiceBootstrap::new(runtime, inputs, trace_prefix)
        .bootstrap()
        .await
}
...
mod tests {
    ...
    async fn bootstrap_registers_s9_s11_services() {
        ...
        let bundle = bootstrap_optional_services(Arc::clone(&runtime), inputs, "test-bootstrap")
            .await
            .unwrap();
    ...
    async fn bootstrap_rejects_duplicate_services() {
        let (runtime, inputs) = test_runtime_and_inputs();
        bootstrap_optional_services(Arc::clone(&runtime), inputs.clone(), "test-bootstrap")
            .await
            .unwrap();

        let error = bootstrap_optional_services(runtime, inputs, "test-bootstrap")
            .await
            .unwrap_err();
    ...
    fn test_runtime_and_inputs() -> (Arc<ServiceRuntime>, OptionalServiceBootstrapInputs) {
        ...
        (
            runtime,
            OptionalServiceBootstrapInputs::new(
                entitlement_store,
                None,
                payment_store,
