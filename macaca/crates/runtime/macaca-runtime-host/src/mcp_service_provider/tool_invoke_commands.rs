...
if let Some(reason) = super::support::validate_invocation_command(&typed) {
    ...
    return Ok(McpSystemServiceProvider::service_result(super::support::to_value(result)?, trace));
}
...
let result = facade
    ...
    .await;
Ok(McpSystemServiceProvider::service_result(super::support::to_value(result)?, trace))
    }
}
