...
pub struct SkillCurationRunRecord {
    ...
    /// Bounded `run.json` artifact reference for replay and operator download.
    ///
    /// The record stores the Store/EventLog ref only.  The JSON body can be
    /// generated by the provider or durable store, but it must not be embedded in
    /// snapshots because curation reports may mention provider diagnostics.
    #[serde(default)]
...
/// Contract tests for governance store DTOs and unavailable Strategy (extracted for file-size gate).
#[cfg(test)]
mod tests;
