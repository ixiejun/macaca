...
fn dispatch_with_bytes(
    ...
    let replay_metadata = random_replay_metadata(name, &payload, stream.is_some());
...
/// Project only bounded random decision facts for the generic audit observer.
/// Generated bytes, tokens, seeds, stream IDs, and provider payloads are never
/// copied into these metadata fields.
fn random_replay_metadata(
    command: &str,
    payload: &Value,
    deterministic: bool,
    ...
    let mut metadata = BTreeMap::from([
        ...
        ("replay.random_command".into(), command.into()),
    ]);
    ...
    {
        metadata.insert(
            "replay.random_length".into(),
            length.min(u64::from(MAX_BYTES)).to_string(),
        );
    }
    metadata
}
