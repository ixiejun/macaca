...
/// Decode a compatibility-tier graph owner from its stable wire label.
///
...
fn compatibility_graph_owner_from_wire_label() -> macaca_proto::TaskGraphOwner {
  let wire_label = ["task", "service", "compatibility"].join("_");
  serde_json::from_value(serde_json::Value::String(wire_label))
    .expect("compatibility graph owner wire label should deserialize")
}

/// Assert a task entry uses compatibility-tier graph owner semantics (not terminal authority).
fn assert_compatibility_graph_owner_semantics(owner: macaca_proto::TaskGraphOwner) {
  assert!(
    !owner.is_application_execution_authoritative(),
    "compatibility graph owner must not drive application-execution terminal aggregation"
  );
  assert_eq!(owner.as_str(), "task_service_compatibility");
}
    ...
    async fn compatibility_graph_is_admitted_but_not_authoritative() {
        ...
        let compatibility = runtime
            .create_task_assignment(CreateTaskAssignmentCommand {
                ...
                depends_on: vec![],
                parent_task: None,
                graph_owner: compatibility_graph_owner_from_wire_label(),
        ...
        assert_compatibility_graph_owner_semantics(compatibility.graph_owner);
        let snapshot = runtime
            .snapshot(TaskServiceSnapshotCommand {
                app_id,
                session_id: Some(session_id.into()),
    ...
    async fn direct_task_space_assignment_defaults_to_native_graph_owner() {
        ...
        let compatibility = space
            .create_task_assignment_with_graph_owner(
                ...
                vec![],
                None,
                compatibility_graph_owner_from_wire_label(),
            )
            .await;
