...
- [x] 1.2 Record the borrowed platform patterns relevant to `pack.ai.embedding.v1` and map them to Macaca descriptors, permissions, policy, service calls, and audit records.
- [x] 1.3 Inventory existing service descriptors, SDK clients, optional packages, plugins, and unavailable providers that can back embedding service provider.
- [x] 1.4 Record GitNexus CRITICAL/HIGH findings as memo only before implementation commits.

## 2. Contract, Descriptor, And Schema

- [x] 2.1 Define provider-neutral command DTOs for: `embedding.embed_text`, `embedding.embed_image`, `embedding.batch_embed`, `embedding.inspect_vector_schema`, `embedding.estimate_cost`.
...
- [x] 3.1 Implement declaration validation for scopes: `ai.embedding.invoke`, `ai.embedding.batch`.
...
- [x] 4.1 Implement or bind embedding service provider through the service runtime; do not construct providers from SDK, shell, kernel, or application code.
...
- [x] 5.1 Extend SDK discovery for `pack.ai.embedding.v1` with command schemas, examples, availability, diagnostics, docs metadata, provider class, and compatibility.
...
- [x] 5.4 Add examples for embed text, embed image, batch embed, inspect vector schema using generic data and without hardcoded application or provider behavior.
...
- [x] 6.2 Add replay tests proving `pack.ai.embedding.v1` calls are trace-addressable through the canonical service path.
...
- [x] 6.5 Run `openspec validate add-pack-ai-embedding --strict`, targeted cargo tests, boundary gates, file-size gates, and audit replay checks before marking complete.

## 7. Developer Documentation

- [x] 7.1 Create or update the detailed developer guide for `pack.ai.embedding.v1` under `docs/developer-packs/`, covering purpose, manifest declaration, permission scopes, command DTOs, result DTOs, examples, unavailable diagnostics, trace/audit behavior, and provider replacement notes.
...
- [x] 7.3 Cross-link the guide from SDK discovery metadata and the industrial pack catalog index before marking `add-pack-ai-embedding` complete.

## 8. Industrial Embedding Semantics

- [x] 8.1 Model `EmbeddingInput`, `EmbeddingBatchRequest`, `EmbeddingVector`, `EmbeddingBatchResult`, `VectorSchemaDescriptor`, and `EmbeddingUsage`.
- [x] 8.2 Add deterministic batch-order tests so output item ids always map to input item ids after retry or partial failure.
- [x] 8.3 Add dimension, numeric type, metric, normalization, and schema compatibility validation.
- [x] 8.4 Add truncation policy tests for text, image, unsupported modality, oversized batch, and per-item denial.
- [x] 8.5 Add idempotent batch retry tests with bounded per-item diagnostics and no raw content in audit.
