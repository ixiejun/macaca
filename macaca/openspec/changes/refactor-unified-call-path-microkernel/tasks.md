...
- [ ] 4.4.4 终态断言：`cargo tree -e normal -p macaca-web --depth 1` 仅 `macaca-sdk`（+ proto DTO/必要 framework 适配）。（**部分**：iteration 120–123 退役 agent/runtime/context/framework；仍冻结 3 条：app/runtime-host/domain-pack-finance）
...
- [ ] 9.4 越界依赖清零：persist 不依赖 context；web/cli 仅依赖 sdk。（persist→context 终态达成：`p2_microkernel_exit_validation` 3/3；CLI 终态达成；web 冻结 3 条 workspace 依赖基线，iteration 120–123 退役 agent/runtime/context/framework；framework 桥接 iter 123 通过 `LlmServiceChatClient` 打破 sdk↔framework 循环）
