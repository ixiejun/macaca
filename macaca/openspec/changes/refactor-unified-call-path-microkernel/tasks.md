...
- [ ] 4.4.4 终态断言：`cargo tree -e normal -p macaca-web --depth 1` 仅 `macaca-sdk`（+ proto DTO/必要 framework 适配）。（**部分**：iteration 120–121 退役 agent/runtime/context；仍冻结 4 条：app/framework/runtime-host/domain-pack-finance）
...
- [ ] 9.4 越界依赖清零：persist 不依赖 context；web/cli 仅依赖 sdk。（persist→context 终态达成：`p2_microkernel_exit_validation` 3/3；CLI 终态达成；web 冻结 4 条 workspace 依赖基线，iteration 120–121 退役 agent/runtime/context）
