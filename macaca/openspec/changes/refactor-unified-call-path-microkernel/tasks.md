...
- [ ] 4.4.4 终态断言：`cargo tree -e normal -p macaca-web --depth 1` 仅 `macaca-sdk`（+ proto DTO/必要 framework 适配）。（**部分**：iteration 120–124 退役 agent/runtime/context/framework/domain-pack-finance；仍冻结 2 条：app/runtime-host）
...
- [x] 6.1.7 实现 `shell-dependency-purity` 审计：CLI 终态仅 proto+sdk；Web 冻结 workspace 依赖基线，禁止新增。（`shell_dependency_purity_gate` via `cargo metadata`；iteration 120–124：7→2）
...
- [x] 7.1 所有新增/迁移模块补详尽英文注释（功能 + 运行原理 + 权衡）；巨型文件拆分后的新文件逐一覆盖。（`english_documentation_terminal_gate` 1/1：composition_bootstrap + runtime-host split + terminal gate 源扫描 0 违规；`factory`/`transport`/`lease`/`contract_source` 模块文档补齐；iteration 119）
...
- [ ] 9.4 越界依赖清零：persist 不依赖 context；web/cli 仅依赖 sdk。（persist→context 终态达成：`p2_microkernel_exit_validation` 3/3；CLI 终态达成；web 冻结 2 条 workspace 依赖基线 `macaca-app` + `macaca-runtime-host`；iteration 124 打破 `app/sdk/pack` 循环，`domain-pack-finance` 经 SDK `domain-pack-finance` feature 桥接；iter 123 退役 framework via `LlmServiceChatClient`）
- [x] 9.5 allowlist == 0；全部终态门绿。（Route C + filesize allowlist==0；`p5_dod_terminal_gate_matrix` 1/1 编排 21 项终态 gate 子进程全绿；web shell 依赖基线冻结 2 条，§9.4 部分达成）
...
- [x] 10.1 在本 change 维护一份 `impact-memo.md`，记录各 `[impact-memo]` task 的 blast radius / risk level，仅备忘不阻塞。（持续维护至 iteration 124）
...
# VC-english-documentation  English module doc 终态（§7.1）
cargo test -p macaca-integration-tests --test english_documentation_terminal_gate -- --nocapture
