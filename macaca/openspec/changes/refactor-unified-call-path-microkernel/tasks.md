...
- [ ] 4.4.4 终态断言：`cargo tree -e normal -p macaca-web --depth 1` 仅 `macaca-sdk`（+ proto DTO/必要 framework 适配）。（**部分**：iteration 120 退役 `macaca-agent`（SDK bridge）+ `macaca-runtime`（dead dep）；仍冻结 5 条：app/context/framework/runtime-host/domain-pack-finance）
...
- [x] 6.1.7 实现 `shell-dependency-purity` 审计：CLI 终态仅 proto+sdk；Web 冻结 5 条 workspace 依赖基线，禁止新增。（`shell_dependency_purity_gate` via `cargo metadata`；iteration 120：7→5）
...
- [ ] 9.4 越界依赖清零：persist 不依赖 context；web/cli 仅依赖 sdk。（persist→context 终态达成：`p2_microkernel_exit_validation` 3/3；CLI 终态达成；web 冻结 5 条 workspace 依赖基线，iteration 120 退役 agent+runtime）
- [x] 9.5 allowlist == 0；全部终态门绿。（Route C + filesize allowlist==0；`p5_dod_terminal_gate_matrix` 1/1 编排 21 项终态 gate 子进程全绿；web shell 依赖基线冻结 5 条，§9.4 部分达成）
