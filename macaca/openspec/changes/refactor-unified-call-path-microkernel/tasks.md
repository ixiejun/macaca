...
### 4.5 P3 退出验证
- [ ] 4.5.1 全仓无 >500 行 OS 层源文件（VC-filesize）。（**部分**：macaca-web + macaca-cli 巨型文件已全部拆分合规；全仓 **62** 行 allowlist 债务（非 shell crate），gate 已实现；iteration 57 退役 `application_manifest.rs`/`memory.rs`/`agent_execution_service`；iteration 56 退役 `governance/pipeline.rs`/`scheduler.rs`/`todo_store.rs`/`executor/queue.rs`/`entitlement.rs`/`execution_control_service`）
