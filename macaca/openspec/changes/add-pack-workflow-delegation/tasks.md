...
- [x] 1.2 Record the borrowed platform patterns relevant to `pack.workflow.delegation.v1` and map them to Macaca descriptors, permissions, policy, service calls, and audit records.
- [x] 1.3 Inventory existing service descriptors, SDK clients, optional packages, plugins, and unavailable providers that can back delegation service provider.
- [x] 1.4 Record GitNexus CRITICAL/HIGH findings as memo only before implementation commits.

## 2. Contract, Descriptor, And Schema

- [x] 2.1 Define provider-neutral command DTOs for: `delegation.delegate`, `delegation.accept_delegation`, `delegation.handoff`, `delegation.inspect_capacity`, `delegation.collect_result`, `delegation.cancel_delegation`.
...
- [x] 4.1 Implement or bind delegation service provider through the service runtime; do not construct providers from SDK, shell, kernel, or application code.
...
- [x] 5.1 Extend SDK discovery for `pack.workflow.delegation.v1` with command schemas, examples, availability, diagnostics, docs metadata, provider class, and compatibility.
...
- [x] 5.4 Add examples for delegate, accept delegation, handoff, inspect capacity using generic data and without hardcoded application or provider behavior.
...
- [x] 7.1 Create or update the detailed developer guide for `pack.workflow.delegation.v1` under `docs/developer-packs/`, covering purpose, manifest declaration, permission scopes, command DTOs, result DTOs, examples, unavailable diagnostics, trace/audit behavior, and provider replacement notes.
...
- [x] 7.3 Cross-link the guide from SDK discovery metadata and the industrial pack catalog index before marking `add-pack-workflow-delegation` complete.
