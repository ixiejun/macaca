...
- [x] 1.2 Record the borrowed platform patterns relevant to `pack.ai.speech.v1` and map them to Macaca descriptors, permissions, policy, service calls, and audit records.
- [x] 1.3 Inventory existing service descriptors, SDK clients, optional packages, plugins, and unavailable providers that can back speech service provider.
- [x] 1.4 Record GitNexus CRITICAL/HIGH findings as memo only before implementation commits.

## 2. Contract, Descriptor, And Schema

- [x] 2.1 Define provider-neutral command DTOs for: `speech.speech_to_text`, `speech.text_to_speech`, `speech.list_voices`, `speech.translate_speech`, `speech.align_timing`.
...
- [x] 3.1 Implement declaration validation for scopes: `ai.speech.recognize`, `ai.speech.synthesize`, `ai.speech.translate`.
...
- [x] 4.1 Implement or bind speech service provider through the service runtime; do not construct providers from SDK, shell, kernel, or application code.
...
- [x] 5.1 Extend SDK discovery for `pack.ai.speech.v1` with command schemas, examples, availability, diagnostics, docs metadata, provider class, and compatibility.
...
- [x] 5.4 Add examples for speech to text, text to speech, list voices, translate speech using generic data and without hardcoded application or provider behavior.
...
- [x] 6.2 Add replay tests proving `pack.ai.speech.v1` calls are trace-addressable through the canonical service path.
...
- [x] 6.5 Run `openspec validate add-pack-ai-speech --strict`, targeted cargo tests, boundary gates, file-size gates, and audit replay checks before marking complete.

## 7. Developer Documentation

- [x] 7.1 Create or update the detailed developer guide for `pack.ai.speech.v1` under `docs/developer-packs/`, covering purpose, manifest declaration, permission scopes, command DTOs, result DTOs, examples, unavailable diagnostics, trace/audit behavior, and provider replacement notes.
...
- [x] 7.3 Cross-link the guide from SDK discovery metadata and the industrial pack catalog index before marking `add-pack-ai-speech` complete.

## 8. Industrial Speech Semantics

- [x] 8.1 Model `SpeechAudioInput`, `SpeechStreamFrame`, `TranscriptSegment`, `VoiceDescriptor`, `SpeechSynthesisRequest`, `SpeechSynthesisResult`, and `SpeechAlignment`.
- [x] 8.2 Add streaming transcription sequence and finalization tests, including late frames and cancellation.
- [x] 8.3 Add diarization, channel, language detection, and word-timing alignment contract tests.
- [x] 8.4 Add voice catalog, voice validation, style, locale, and output-format compatibility tests.
- [x] 8.5 Add redaction tests proving raw audio, generated speech bytes, credentials, and provider payloads never enter observability.
