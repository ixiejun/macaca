...
- [ ] 4.1 Implement the device notification service provider contract behind the service runtime; do not construct providers from kernel, SDK, shells, or generic application-framework code.
...
- [ ] 6.3 Add dependency-boundary gates proving microkernel, SDK, shells, and generic application framework do not import concrete notification providers or host notification APIs.
- [ ] 6.4 Add no-direct-provider-call gates proving all notification commands enter through descriptor-owned service registrations and typed service runtime dispatch.
- [ ] 6.5 Add redaction tests for title/body text, action input, push tokens, provider payloads, credentials, notification history, interaction events, snapshots, and diagnostics.
