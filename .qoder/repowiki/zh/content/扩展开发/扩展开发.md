...
- [macaca/docs/agent-os-mcp-framework-plan.md](file://macaca/docs/agent-os-mcp-framework-plan.md)
- [macaca/docs/agent-skills-runtime.md](file://macaca/docs/agent-skills-runtime.md)
...
- [macaca/crates/macaca-skill/src/runtime.rs](file://macaca/crates/macaca-skill/src/runtime.rs)
...
- [macaca/crates/macaca-app/src/runtime.rs](file://macaca/crates/macaca-app/src/runtime.rs)
- [macaca/crates/macaca-kernel/src/lib.rs](file://macaca/crates/macaca-kernel/src/lib.rs)
- [macaca/crates/macaca-web/src/mcp_runtime.rs](file://macaca/crates/macaca-web/src/mcp_runtime.rs)
- [macaca/crates/macaca-web/src/skill_mcp.rs](file://macaca/crates/macaca-web/src/skill_mcp.rs)
- [macaca/examples/apps/fullstack-autodev/app.yaml](file://macaca/examples/apps/fullstack-autodev/app.yaml)
</cite>

## 更新摘要
**所做更改**
- 更新了 MCP 框架架构，反映系统级 MCP 运行时和 Agent Skills 运行时的职责分离
- 新增了 Agent OS MCP Registry 和 Runtime 的详细说明
- 更新了技能系统与 MCP 服务器解耦的设计理念
- 增强了框架层 MCP 协议支持和传输抽象
- 更新了工具注册和生命周期管理的新架构
...
- 自定义组件的开发、注册与集成方法
- **新增** MCP（Model Context Protocol）系统级运行时架构，包括职责分离和解耦设计
...
- **框架层**：提供统一的消息类型、工具系统、记忆体、管道编排和 MCP 协议支持
- **MCP 层**：提供 MCP 客户端、适配器与驱动，支持多种传输协议
- **技能层**：提供可执行技能与知识型技能（agentskills.io）管理
- **应用层**：声明式应用模型，支持 L1/L2/L3 层次的应用加载与运行
- **内核层**：调度、执行、编排与服务桥接
- **Web 层**：Agent OS 级 MCP Registry 和 Runtime，实现系统级服务管理
- **示例与文档**：示例应用、Persona/Workflow 配置与系统文档

```mermaid
graph TB
subgraph "框架层"
FW["macaca-framework<br/>消息/工具/MCP协议/A2A/MCP"]
end
subgraph "MCP 层"
...
subgraph "Web 层"
WEB["macaca-web<br/>MCP Registry/Runtime/策略"]
end
subgraph "代理层"
...
- [macaca/crates/macaca-web/src/mcp_runtime.rs:1-39](file://macaca/crates/macaca-web/src/mcp_runtime.rs#L1-L39)
...
- **MCP 系统级运行时**：支持 stdio、SSE、streamable HTTP 传输协议，实现 OS 级 MCP 服务器管理
...
- [macaca/crates/macaca-app/src/lib.rs:1-21](file://macaca/crates/macaca-app/src/lib.rs#L1-L21)
- [macaca/crates/macaca-kernel/src/lib.rs:1-29](file://macaca/crates/macaca-kernel/src/lib.rs#L1-L29)
- [macaca/crates/macaca-web/src/mcp_runtime.rs:1-800](file://macaca/crates/macaca-web/src/mcp_runtime.rs#L1-L800)

## 架构总览
Agent OS 以"框架-应用-内核-代理"分层协作，新增了系统级 MCP 运行时：
- **框架层**提供统一的工具、消息、记忆与 MCP 集成
- **Web 层**实现 Agent OS 级 MCP Registry 和 Runtime，管理 MCP 服务器生命周期
- **应用层**以声明式方式描述 Agent 组织、工作流与资源路径
- **内核层**负责任务调度、执行与编排
- **代理层**封装具体 Agent 行为与生命周期
...
subgraph "Web 层"
REG["MCP Registry<br/>服务器定义/策略"]
RUNTIME["MCP Runtime<br/>生命周期管理/实例控制"]
POLICY["策略过滤<br/>可见性/权限"]
end
subgraph "内核层"
SCH["调度器"]
EXE["执行器"]
...
### MCP 系统级运行时架构
**更新** 新增系统级 MCP 运行时架构，实现职责分离和解耦设计

- **协议与消息**
  - **传输支持**：Stdio（子进程 JSON-RPC over stdin/stdout）、SSE（HTTP Server-Sent Events）、Streamable HTTP
  - **工具定义**：名称、描述、输入 Schema
  - **工具结果**：内容块（文本/图片/资源）与 isError 标记
- **系统级架构**
  - **MCP Registry**：管理 OS 级 MCP 服务器定义、配置和状态
  - **MCP Runtime**：按生命周期模式管理实例、连接池、健康检查、超时、资源释放
  - **策略过滤**：基于应用/代理策略控制服务器和工具的可见性
- **关键流程（MCP 工具注册）**

```mermaid
sequenceDiagram
participant App as "应用/Agent"
participant Runtime as "McpRuntimeManager"
participant Registry as "MCP Registry"
participant Client as "McpClient"
participant Toolkit as "Toolkit"
App->>Runtime : "register_tools(policy, context)"
Runtime->>Registry : "获取服务器定义"
Registry-->>Runtime : "McpServerDefinition[]"
Runtime->>Runtime : "应用策略过滤"
Runtime->>Client : "创建客户端实例"
Client-->>Runtime : "连接成功"
Runtime->>Client : "list_tools()"
Client-->>Runtime : "McpToolDef[]"
Runtime->>Toolkit : "register_mcp_tools_with_options()"
Toolkit-->>Runtime : "注册完成"
Runtime-->>App : "状态报告"
```

**图表来源**
- [macaca/crates/macaca-web/src/mcp_runtime.rs:280-322](file://macaca/crates/macaca-web/src/mcp_runtime.rs#L280-L322)
- [macaca/crates/macaca-framework/src/mcp.rs:934-942](file://macaca/crates/macaca-framework/src/mcp.rs#L934-L942)
...
**更新** 技能系统与 MCP 服务器实现解耦，MCP 服务器职责明确分离
...
- **知识技能（agentskills.io）**
  ...
  - **职责边界**：仅提供知识和使用说明，不拥有 MCP 进程生命周期
- **关键流程（技能加载与工具化）**
...
ToolkitReg --> End(["完成"])
MCPStart(["MCP 服务器发现"]) --> ParseMetadata["解析技能元数据"]
ParseMetadata --> ExtractServers["提取 MCP 服务器配置"]
ExtractServers --> RegisterToRegistry["注册到 MCP Registry"]
RegisterToRegistry --> EndMCP(["MCP 服务器就绪"])
```
...
- [macaca/docs/agent-skills-runtime.md:45-55](file://macaca/docs/agent-skills-runtime.md#L45-L55)

**章节来源**
- [macaca/crates/macaca-skill/src/lib.rs:1-35](file://macaca/crates/macaca-skill/src/lib.rs#L1-L35)
- [macaca/crates/macaca-skill/src/definition.rs:1-141](file://macaca/crates/macaca-skill/src/definition.rs#L1-L141)
- [macaca/crates/macaca-skill/src/registry.rs:1-267](file://macaca/crates/macaca-skill/src/registry.rs#L1-L267)
- [macaca/docs/agent-skills-runtime.md:1-115](file://macaca/docs/agent-skills-runtime.md#L1-L115)
...
## 依赖关系分析
**更新** 新增 Web 层与 MCP 运行时的依赖关系

- **工作区成员与依赖**
  - 框架层依赖消息、工具、Proto、IPC 等基础能力
  - Web 层依赖框架的 MCP 协议和工具系统
  - MCP 层依赖框架的消息与工具系统
  - 技能层依赖 Proto、Tools
  - 应用层依赖框架与技能
  - 内核层依赖应用、框架与服务桥接
- **模块耦合**
  ...
  - **Web 层**作为系统级 MCP 管理器，通过框架 MCP 协议与外部服务解耦
  - 技能系统与 MCP 服务器实现职责分离，通过 Registry 解耦
  - 应用层通过声明式配置与内核/框架交互
...
## 性能考虑
**更新** 新增 MCP 运行时架构的性能考虑

- **工具调用链**
  - 中间件数量与复杂度直接影响延迟；建议按需启用组与中间件
  - 参数 Schema 校验与预处理应轻量，避免阻塞
- **MCP 交互**
  - **系统级连接池**：使用共享客户端与适配器，减少重复握手与连接开销
  - **生命周期管理**：按生命周期模式管理实例，避免资源泄漏
  - **策略过滤**：在注册前应用策略，减少不必要的工具注册
  - 结果聚合与内容块序列化应避免不必要的拷贝
- **记忆体与缓存**
  - 合理设置缓存策略与失效时间，避免过期数据占用内存
- **并发与调度**
  - 内核执行队列与路由策略应平衡吞吐与延迟
  - **MCP 实例隔离**：按会话/代理分配实例，避免状态串扰

## 故障排查指南
**更新** 新增 MCP 运行时架构的故障排查指南
...
- **MCP 相关**
  - **系统级问题**：检查 MCP Registry 配置文件和服务器定义
  - **生命周期问题**：确认实例引用计数和清理策略
  - **策略过滤问题**：验证应用/代理策略配置
  - **传输问题**：确认网络连接、URL 和认证信息
  - **工具不存在**：确认服务器已列出该工具或手动注册
  - **错误响应**：查看 is_error 标记与内容块中的错误文本
- **技能相关**
  - YAML 解析失败：检查语法与字段完整性
  - 目录不存在：确认技能目录路径正确
  - **MCP 服务器发现失败**：检查技能元数据中的 MCP 配置
- **应用与工作流**
...
- [macaca/crates/macaca-web/src/mcp_runtime.rs:485-546](file://macaca/crates/macaca-web/src/mcp_runtime.rs#L485-L546)
- [macaca/examples/apps/fullstack-autodev/app.yaml:65-146](file://macaca/examples/apps/fullstack-autodev/app.yaml#L65-L146)

## 结论
**更新** 本指南梳理了 Agent OS 的扩展开发路径，重点反映了新的 MCP 系统级运行时架构：

- 以工具系统为核心，通过 MCP 与技能系统对接外部生态
- **新增** 系统级 MCP Registry 和 Runtime 实现职责分离，MCP 服务器管理与应用解耦
- **新增** 技能系统与 MCP 服务器实现职责边界，知识技能仅提供使用说明
- 借助声明式应用模型与内核调度实现可维护、可扩展的多 Agent 执行系统
- 遵循本文档的最佳实践与测试方法，可高效构建高质量扩展

## 附录

### 扩展开发最佳实践
**更新** 新增 MCP 运行时架构的最佳实践
...
- **自定义组件**
  ...
  - 将工具注册到合适的组，按需启用/停用
- **MCP 系统级集成**
  - **优先使用系统级 MCP Registry**：通过 OS 级配置管理 MCP 服务器
  - **实现生命周期感知**：正确处理 stateful/stateless 生命周期模式
  - **应用策略过滤**：在注册前应用应用/代理策略，减少不必要的工具暴露
  - **传输协议选择**：根据需求选择 stdio、SSE 或 streamable HTTP
  - **实例隔离**：按会话/代理分配实例，避免状态串扰
- **技能系统**
  - 可执行技能使用 YAML 明确声明入口点与参数
  - 知识技能遵循 agentskills.io 规范，提供清晰的目录结构与资源
  - **职责分离**：技能仅提供知识说明，不直接管理 MCP 服务器生命周期
- **第三方服务集成**
  - 通过适配器/工具包装第三方 API，统一错误处理与数据转换
  - 在工具响应中附带元数据（如 token 数、耗时），便于监控与优化
  - **系统级配置**：优先使用 OS 级 MCP Registry 管理外部服务
- **测试与发布**
  - 单元测试覆盖工具调用、中间件链、MCP 客户端行为
  - **集成测试**：验证 MCP Registry、Runtime 和工具注册流程
  - **系统级测试**：验证应用清单、工作流与 Agent 生命周期
  - 发布前确保依赖版本一致、文档与示例同步更新
  - **架构验证**：确保 MCP 服务器与技能系统职责分离得到正确实现