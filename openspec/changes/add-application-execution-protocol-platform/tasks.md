...
- [x] 7.1 Extend application manifest parsing or execution profile metadata to declare an external backend endpoint, protocol version, callback identity reference, supported controls, heartbeat interval, timeout, and event schema version.
...
- [x] 12.5 Update `macaca/docs/macaca-os-serviceization-allowlist.md` only if this implementation adds, removes, or narrows a real dependency exception; do not update it cosmetically.
- [x] 12.6 Update `macaca/docs/macaca-os-architecture-governance.md` and `macaca/docs/macaca-os-microkernel-boundaries.md` only if this implementation adds stable ownership language that is not already covered.
