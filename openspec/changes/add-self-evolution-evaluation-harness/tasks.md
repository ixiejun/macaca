...
- [x] 2.4 Add structured logs for checkpoint append, scoring, report creation,
...
- [x] 4.1 Observe verified task completion, ExperienceCandidate,
...
- [x] 4.2 Ensure evaluation checkpoints store refs and bounded counts only.
- [x] 4.3 Add rollback and before/after snapshot refs to evaluation reports.

## 5. Shell Adapters And Runbook

- [x] 5.1 Add Web/CLI route or command adapters that display evaluation reports
  without owning scoring semantics.
- [x] 5.2 Add frontend report display only if the existing operations surface
  needs visibility into evaluation status. Not required for this slice because
  the Web route and runbook provide operator visibility without frontend UI
  changes.
- [x] 5.3 Document how to run baseline/evolved task-family evaluations.

## 6. Verification

- [x] 6.1 Test white-box gate success and each required missing-checkpoint
  failure.
- [x] 6.2 Test black-box pass, no-activation failure, completion regression
